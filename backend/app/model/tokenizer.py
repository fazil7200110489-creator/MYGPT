...
class BPETokenizer:
    ...
    def decode(self, ids: List[int], skip_special: bool = True) -> str:
        """Converts token IDs back to a readable text string with deduplication filtering."""
        tokens = []
        for idx in ids:
        ...
        deduped_words = []
        for w in words:
            # Fix repeating syllables/words in single word tokens (e.g. "HereHere" -> "Here", "summarysummary" -> "summary")
            if len(w) > 2 and len(w) % 2 == 0:
                half = len(w) // 2
                if w[:half] == w[half:]:
                    w = w[:half]
            
            # Avoid repeating adjacent identical words (e.g. "the the" -> "the")
            if not deduped_words or w != deduped_words[-1]:
                deduped_words.append(w)
                
        decoded_clean = " ".join(deduped_words)
        
        # Avoid repeating adjacent identical sentences
        sentences = [s.strip() for s in re.split(r"(?<=\.|\?|!)\s+", decoded_clean) if s.strip()]
        deduped_sentences = []
        for s in sentences:
