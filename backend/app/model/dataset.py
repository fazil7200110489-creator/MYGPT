...
class TextDataset(Dataset):
    ...
    def _read_file(self, filepath: str) -> str:
        """Reads text from a file or directory with UTF-8 encoding.

        Consolidates multiple files if directory, ignores empty lines, removes duplicate
        blank spaces, and preserves paragraphs.
        """
        import os
        import re

        # Directory loading case
        if os.path.isdir(filepath):
            combined_paragraphs = []
            files = [os.path.join(filepath, f) for f in os.listdir(filepath) if f.endswith(".txt")]
            if not files:
                raise FileNotFoundError(f"No .txt files found in directory: {filepath}")
            ...
            for fp in sorted(files):
                ...
                for para in raw_paras:
                    ...
                    if para_text:
                        combined_paragraphs.append(para_text)
